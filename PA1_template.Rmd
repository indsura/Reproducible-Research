...
groupedbyinterval<-group_by(transformeddf,interval)
## Finding out the mean per interval
meanperinterval<-summarize_all(groupedbyinterval,mean)
...
### This phenomenon occurs, because we have imputed the values with the mean of those respective
### intervals. Interval 0 gets the mean of the interval 0 and so on and so forth. Calculating
### the average value/ measure of central tendency is just strengthened by imputing the missing
### values with the mean (again, a measure of central tendency). New mean:
...
### Answer b): Previous median: 10765, New median: 10766.19. Therefore, the median changes.
### By replacing the missing values with the mean, we are increasing the count of the most 
### occuring values and hence increasing the median.New median: 
```{r}
